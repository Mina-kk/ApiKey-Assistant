...
├── scripts/            # 构建辅助脚本
│   └── sync-version.js # 版本号同步工具（构建时自动同步版本）
...
└── www/                # 前端源码
    ...
    ├── css/style.css   # 全局样式（深色/浅色主题、移动端适配）
    ├── img/            # 背景图与应用图标
    └── js/
        ...
        ├── newapi.js   # NewAPI 入口（模型命名 + 令牌查询下拉菜单）
        ├── update.js   # 自动更新检查与关于页面
        ├── token-query.js # 令牌查询统计工具（NewAPI 中转站用量查询与图表）
...
- **NewAPI 工具集** — 模型命名（后缀重命名 + 重定向 JSON 生成）+ 令牌查询（中转站用量统计与图表）
...
- **主题切换** — 深色、浅色、跟随系统
- **本地代理检测** — 自动检测代理状态与延迟
- **令牌隐私保护** — Base URL 和 Key 可切换显示/隐藏，默认掩码保护
...
```bash
npm run build          # 构建 release APK（自动同步版本号）
npm run build-debug    # 构建 debug APK（自动同步版本号）
npm run clean          # 清理构建缓存
npm run sync-version   # 手动同步版本号到所有配置文件
```
...
| 版本 | 日期 | 变更 |
|------|------|------|
| **v3.0.6** | 2026-06-14 | 新增令牌查询统计工具（NewAPI 中转站用量查询与图表）；NewAPI 入口改为下拉菜单（模型命名/令牌查询）；新增版本同步脚本 scripts/sync-version.js；主题循环修复（dark/light/auto） |
| **v3.0.5** | 2026-06-14 | 新增 Base URL 和 Key 的显示/隐藏切换按钮；maskKey 改为 sk- 前缀全掩码；新增 maskBaseUrl 函数 |
| **v3.0.42** | 2026-06-14 | 更新弹窗自动关闭倒计时（10s），替代 confirm 对话框 |
| **v3.0.41** | 2026-06-14 | 新增自动更新检查与关于页面 |
| **v3.0.3** | 2026-06-14 | 新增模型获取方式选择与批量超时控制 |
| **v3.0.2** | 2026-06-14 | 渠道测试直连、运行日志精简 |
| **v3.0.1** | 2026-06-19 | 新增批量移动分组、NewAPI 模型命名工具 |
| **v3.0.0** | 2026-06-16 | 首个正式发布 |
